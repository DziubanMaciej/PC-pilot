...
class ServerMessage private constructor(bytes: ByteArray, val address: InetSocketAddress) : Message(bytes) {
    ...
    companion object {
        ...
        fun createMessageKeyPress(address: InetSocketAddress, unicode: Int): ServerMessage {
            val bytes = ByteBuffer.allocate(SIZE)
                    .order(ByteOrder.LITTLE_ENDIAN)
                    .putPreamble()
        ...
        fun createMessageKeyPressBackspace(address: InetSocketAddress): ServerMessage {
            val bytes = ByteBuffer.allocate(SIZE)
                    .order(ByteOrder.LITTLE_ENDIAN)
                    .putPreamble()
                    .put(Type.KeyPressBackspace.value)
                    .array()
            return ServerMessage(bytes, address)
        }
    }
    ...
    fun getMessageKeyPressUnicode(): Int {
        return getInt(8)
    }
}
...
class ClientMessage private constructor(bytes: ByteArray, val address: InetSocketAddress) : Message(bytes) {
    ...
    companion object {
        ...
        fun createMessageAdvertise(address: InetSocketAddress): ClientMessage {
            val bytes = ByteBuffer.allocate(SIZE)
                    .order(ByteOrder.LITTLE_ENDIAN)
                    .putPreamble()
                    .put(Type.Advertise.value)
                    .array()
            return ClientMessage(bytes, address)
        }
    }
